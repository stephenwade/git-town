...
func TestTestCommands(t *testing.T) {
	...
	t.Run("AddRemote", func(t *testing.T) {
		...
		remotes, err := dev.Remotes(dev.TestCommands)
		must.NoError(t, err)
	...
	t.Run("CreateBranch", func(t *testing.T) {
		t.Run("simple branch name", func(t *testing.T) {
			...
			currentBranch, err := runtime.CurrentBranch(runtime.TestCommands)
			must.NoError(t, err)
	...
	t.Run("HasBranchesOutOfSync", func(t *testing.T) {
		t.Run("branches are in sync", func(t *testing.T) {
			...
			runner.PushBranchToRemote(gitdomain.NewLocalBranchName("branch1"), gitdomain.RemoteOrigin)
